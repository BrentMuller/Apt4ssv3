/* FROM.f -- translated by f2c (version 20100827).
...
/* .....FORTRAN SUBROUTINE  ....FROM           8/68                 AR,PH */

/* Subroutine */ int from_(doublereal *point)
{
    /* Initialized data */

    static char hfrom[4] = "FROM";
...
#define sinl ((doublereal *)&tlldat_1 + 8)
    extern /* Subroutine */ int apt202_(doublereal *, integer *), apt233_(
	    void), transm_(doublereal *, doublereal *, integer *, integer *, 
	    integer *);
...
/* ...  THE PRESENT TOOL POSITION IS (POINT) */
...
/*      INTEGER LITERALS */
    ...
    ifxcor_1.iaerr = 0;
    ifxcor_1.igoto = 0;
    transm_(fxcor_1.tek, &point[1], &c__2, &c__3, &c__0);
    for (j = 1; j <= 3; ++j) {
/* L10: */
	fxcor_1.te[j - 1] = fxcor_1.tek[j - 1];
    }
    apt202_(&point[1], &c__2);
/* ...  SET RECORD TYPE CODE FOR CLTAPE WRITER */
    s_copy(charct_1.modfer, hfrom, (ftnlen)6, (ftnlen)4);
    apt233_();
    return 0;
} /* from_ */
