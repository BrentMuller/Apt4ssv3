/* INDIRP.f -- translated by f2c (version 20100827).
...
static integer c__20901 = 20901;
static integer c__20902 = 20902;
static integer c_b25 = -20903;

/* *** SOURCE FILE : M0002754.V09   *** */


/* .....FORTRAN SUBROUTINE             INDIRP...              3/1/68   GK */
/* Subroutine */ int indirp_(doublereal *srfc, doublereal *pnt)
{
...
#define sinl ((doublereal *)&tlldat_1 + 8)
    static char test[8];
#define point ((doublereal *)&hsave_1 + 32)
    extern /* Subroutine */ int vnorm_(doublereal *, doublereal *);
#define srface ((doublereal *)&hsave_1 + 31)
...
/* ...  NEW DIRECTION OF MOTION IS FROM PRESENT TOOL POSITION TO POINT */
/* ...  STORE DIRECTION VECTORS FOR DS,PS,CS AS REQUIRED */
    ...
    *srface = *srfc;
    ...
    transm_(point, &pnt[1], &c__2, &c__3, &c__0);
    if (hold_1.jhldfg == 0) {
	goto L501;
    }
    hold_1.itabh[hold_1.ithcnt - 1] = 12;
    ++hold_1.ithcnt;
    return 0;
...
L5:
    for (i__ = 1; i__ <= 3; ++i__) {
	sv_1.vndir[i__ + k * 3 - 4] = point[i__ - 1] - fxcor_1.te[i__ - 1];
/* L10: */
    }
    vnorm_(&sv_1.vndir[k * 3 - 3], &sv_1.vndir[k * 3 - 3]);
    if (ifxcor_1.ier <= 0) {
	goto L25;
    } else {
	goto L20;
...
L20:
    aercom_1.iaerfg = 1;
    aerr_(&c__20901, "INDIRP  ", (ftnlen)8);
    goto L45;
L25:
    isv_1.indir__[k - 1] = knumbr_1.k1;
/* ...  IF DS,TELL AMIND VNDIR IS ALREADY THE FORWARD DIRECTION */
    if (s_cmp(hblock_1.hds, test, (ftnlen)8, (ftnlen)8) != 0) {
	goto L1000;
    }
/* L30: */
    for (i__ = 1; i__ <= 3; ++i__) {
	fxcor_1.ti[i__ - 1] = sv_1.vndir[i__ + k * 3 - 4];
	fxcor_1.tek[i__ - 1] = fxcor_1.te[i__ - 1] - fxcor_1.ti[i__ - 1];
/* L35: */
    }
L1000:
    return 0;
/* ...  DIRECTION OF MOTION GIVEN WITHOUT A STARTING POINT GIVEN */
L40:
    aercom_1.iaerfg = 1;
    aerr_(&c__20902, "INDIRP  ", (ftnlen)8);
L45:
    ifxcor_1.iaerr = 0;
    goto L1000;
/* ...  WARNING-CANNOT DETERMINE SURFACE TYPE. DS ASSUMED */
L50:
    aerr_(&c_b25, "INDIRP  ", (ftnlen)8);
...
/* Subroutine */ int indrph_(doublereal *srfc, doublereal *pnt)
{
    /* System generated locals */
    static doublereal equiv_2[1];
...
L20:
    aercom_1.iaerfg = 1;
    aerr_(&c__20901, "INDRPH  ", (ftnlen)8);
    goto L45;
L25:
    isv_1.indir__[k - 1] = knumbr_1.k1;
/* ...  IF DS,TELL AMIND VNDIR IS ALREADY THE FORWARD DIRECTION */
...
L40:
    aercom_1.iaerfg = 1;
    aerr_(&c__20902, "INDRPH  ", (ftnlen)8);
L45:
    ifxcor_1.iaerr = 0;
    goto L1000;
/* ...  WARNING-CANNOT DETERMINE SURFACE TYPE. DS ASSUMED */
L50:
    aerr_(&c_b25, "INDRPH  ", (ftnlen)8);
    ihds[0] = bcdf_(hblock_1.hds, (ftnlen)4);
    ihds[1] = bcdf_(hblock_1.hds + 4, (ftnlen)4);
    fxcor_1.temp[0] = *valhds;
    goto L3;
