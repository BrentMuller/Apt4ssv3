/* AWRITE.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int awrite_(integer *no, integer *irecno, real *data, 
	integer *isize, integer *iret)
{
    ...
    static char darray[80] = "                                              "
	    "                                  ";
    static real dum[256] = { 0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,
...
/* L20: */
		...
		*iret = s_wdue(&io___15);
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    *iret = do_uio(&c__1, (char *)&dum[i__ - 1], (ftnlen)
			    sizeof(real));
		    if (*iret != 0) {
			goto L90;
		    }
...
/* ---- SEQUENTIAL ACCESS FILE */
	...
	i__1 = *isize;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    do_uio(&c__1, (char *)&data[i__], (ftnlen)sizeof(real));
	}
	e_wsue();
    }
    return 0;
L90:
    slen = 0;
    cform_(" *** ERROR WRITING BY AWRITE ON FILE :", darray, &c__2, &slen, (
	    ftnlen)38, (ftnlen)80);
    posn = slen + 3;
    slen = 0;
